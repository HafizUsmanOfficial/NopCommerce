...
using Nop.Core.Domain.XmlValue;
...
namespace Nop.Services.Orders
{
    ...
    public partial class ShoppingCartService : IShoppingCartService
    {
        ...
        public virtual void MigrateShoppingCart(Customer fromCustomer, Customer toCustomer, bool includeCouponCodes)
        {
            ...
            //Merge Code if Exits item Customer Id Shoping cart

            List<XmlValues> previousitemIDList = new List<XmlValues>();
            List<XmlValues> currentitemIDList = new List<XmlValues>();
            var filnalList = new List<XmlValues>();
            // LIST previous items

            if (toCart.Count > 0 && fromCart.Count > 0)
            {
                string xml = toCart[0].AttributesXml;
                XDocument doc = XDocument.Parse(xml);

                foreach (var name in doc.Descendants("Attributes").Elements())
                {
                    XmlValues xmlvalue = new XmlValues();
                    xmlvalue.ID = ((int)name.Attribute("ID"));
                    xmlvalue.Name = name.Element("ProductAttributeValue").Value;
                    previousitemIDList.Add(xmlvalue);

                }


                //shopping cart items
                string xmlSecond = fromCart[0].AttributesXml;
                XDocument docsec = XDocument.Parse(xmlSecond);
                foreach (var name in docsec.Descendants("Attributes").Elements())
                {
                    XmlValues xmlvalue = new XmlValues();
                    xmlvalue.ID = ((int)name.Attribute("ID"));
                    xmlvalue.Name = name.Element("ProductAttributeValue").Value;
                    currentitemIDList.Add(xmlvalue);


                }

                //Name And Quantity Update second items
                List<int> NameAndQuantityList = new List<int>()
                {
                    6,9,11,12,15,16,18,19,21,22,24,25,27,28,30,31,33,34,36,37,39,
                    40,42,43,45,46,48,49,51,52,54,55,57,58,60,61,63,64,66,67,69,
                    70,72,73,75,76,78,79,81,82
                };
                var LastQuantityID = 0;
                var ExitNameAndQuantity = previousitemIDList.Where(t => NameAndQuantityList.Contains(t.ID)).ToList();
                var LastEnterQuantityID = ExitNameAndQuantity.LastOrDefault().ID;
                if (LastEnterQuantityID == 12)
                {
                    LastQuantityID = LastEnterQuantityID + 3;
                }
                else
                {
                    LastQuantityID = LastEnterQuantityID + 2;
                }

                List<XmlValues> newNameAndQuantitylist = new List<XmlValues>();
                //update item IDs in cart
                for (int i = LastQuantityID; i == LastQuantityID;)
                {
                    for (int j = 1; j < currentitemIDList.Count; j++)
                    {
                        if (currentitemIDList[j].ID == 6 || currentitemIDList[j].ID == 9 || currentitemIDList[j].ID == 11 || currentitemIDList[j].ID == 12
                            || currentitemIDList[j].ID == 15 || currentitemIDList[j].ID == 16 || currentitemIDList[j].ID == 18 || currentitemIDList[j].ID == 19
                            || currentitemIDList[j].ID == 21 || currentitemIDList[j].ID == 22 || currentitemIDList[j].ID == 24 || currentitemIDList[j].ID == 25
                            || currentitemIDList[j].ID == 27 || currentitemIDList[j].ID == 28 || currentitemIDList[j].ID == 30 || currentitemIDList[j].ID == 31
                            || currentitemIDList[j].ID == 33 || currentitemIDList[j].ID == 34 || currentitemIDList[j].ID == 36 || currentitemIDList[j].ID == 37
                            || currentitemIDList[j].ID == 39 || currentitemIDList[j].ID == 40 || currentitemIDList[j].ID == 42 || currentitemIDList[j].ID == 43
                            || currentitemIDList[j].ID == 45 || currentitemIDList[j].ID == 46 || currentitemIDList[j].ID == 48 || currentitemIDList[j].ID == 49
                            || currentitemIDList[j].ID == 51 || currentitemIDList[j].ID == 52 || currentitemIDList[j].ID == 54 || currentitemIDList[j].ID == 55
                            || currentitemIDList[j].ID == 57 || currentitemIDList[j].ID == 58 || currentitemIDList[j].ID == 60 || currentitemIDList[j].ID == 61
                            || currentitemIDList[j].ID == 63 || currentitemIDList[j].ID == 64 || currentitemIDList[j].ID == 66 || currentitemIDList[j].ID == 67
                            || currentitemIDList[j].ID == 69 || currentitemIDList[j].ID == 70 || currentitemIDList[j].ID == 72 || currentitemIDList[j].ID == 73
                            || currentitemIDList[j].ID == 75 || currentitemIDList[j].ID == 76 || currentitemIDList[j].ID == 78 || currentitemIDList[j].ID == 79
                            || currentitemIDList[j].ID == 81 || currentitemIDList[j].ID == 82)
                        {
                            if (i==14)
                            {
                                i++;
                            ...
                            XmlValues xmlv = new XmlValues();
                            xmlv.ID = i;
                            xmlv.Name = currentitemIDList[j].Name;
                            newNameAndQuantitylist.Add(xmlv);
                            i++;
                        }
                        else
                        {
                ...
                var NextColorID = 0;
                if (LastEnterQuantityID == 13 )
                {
                    NextColorID = LastEnterQuantityID + 5;
                }
                else
                {
                    NextColorID = LastEnterQuantityID + 4;
                }

                // update new cart colort ID
                List<XmlValues> newColorlist = new List<XmlValues>();
                for (int i = NextColorID; i == NextColorID;)
                {
                    for (int j = 3; j < currentitemIDList.Count; j++)
                    {
                        if (currentitemIDList[j].ID == 10 || currentitemIDList[j].ID == 13 || currentitemIDList[j].ID == 17 || currentitemIDList[j].ID == 20
                            || currentitemIDList[j].ID == 23 || currentitemIDList[j].ID == 26 || currentitemIDList[j].ID == 29 || currentitemIDList[j].ID == 32
                            || currentitemIDList[j].ID == 35 || currentitemIDList[j].ID == 38 || currentitemIDList[j].ID == 41 || currentitemIDList[j].ID == 44
                            || currentitemIDList[j].ID == 47 || currentitemIDList[j].ID == 50 || currentitemIDList[j].ID == 53 || currentitemIDList[j].ID == 56
                            || currentitemIDList[j].ID == 59 || currentitemIDList[j].ID == 62 || currentitemIDList[j].ID == 65 || currentitemIDList[j].ID == 68
                            || currentitemIDList[j].ID == 71 || currentitemIDList[j].ID == 74 || currentitemIDList[j].ID == 77 || currentitemIDList[j].ID == 80
                            || currentitemIDList[j].ID == 83)
                        {
                            if (i==16)
                            {
                                i++;
                ...
                var exitRecordAddColor = LastEnterQuantityID + 1;
                foreach (var item in previousitemIDList)
                {
                    if (item.ID == exitRecordAddColor)
                    {
                        XmlValues xmlv = new XmlValues();
                        xmlv.ID = item.ID;
                        xmlv.Name = item.Name;
                        ExitNameAndQuantity.Add(xmlv);

                    }
                }
                var MergeSecondRecordListWithColor = newNameAndQuantitylist.Concat(newColorlist).OrderBy(x => x.ID);

                //var list = MergeSecondRecordListWithColor.Except(
                //    MergeSecondRecordListWithColor.Where(o => previousitemIDList.Select(s => s.ID).ToList().Contains(o.ID))).ToList();


                var bothListMerge = MergeSecondRecordListWithColor.Union(previousitemIDList).OrderBy(x => x.ID).ToList();
                filnalList = bothListMerge.GroupBy(x=>x.ID).Select(g=>g.First()).ToList();
                //var mergeTwoListWithoutColor = newNameAndQuantitylist.Concat(ExitNameAndQuantity).OrderBy(x => x.ID).ToList();
                //var TotalQuantity = mergeTwoListWithoutColor.Count();
                //TotalQuantity = TotalQuantity - 2;
                //TotalQuantity = TotalQuantity / 2;
                //finalList[0].Name = TotalQuantity.ToString(); 
                //var mergeWithColor = mergeTwoListWithoutColor.Concat(newColorlist).OrderBy(x=>x.ID);
                //var finalList = MergeSecondRecordListWithColor.Intersect(previousitemIDList).ToList();
                ////var finalList = newlist.Concat(currentitemIDList).Where(x=>x.ID==);
                ///
                var PreviousQuantity = toCart[0].Quantity;
                var Currentquantity = fromCart[0].Quantity;
                var TotalQuantity = PreviousQuantity + Currentquantity;
                toCart[0].Quantity = TotalQuantity;
                filnalList[0].Name = TotalQuantity.ToString();
                //var mergList = previousitemIDList.Concat(newlist).OrderBy(x => x.ID);
                var xEle = new XElement("Attributes",
                                from m in filnalList
                                select new XElement("ProductAttribute",
                                             new XAttribute("ID", m.ID),
                                             new XElement("ProductAttributeValue", new XElement("Value", m.Name))
                                           //new XElement("Value", m.Name)
                                           ));
                toCart[0].AttributesXml = xEle.ToString();
            }
            ...
            if (filnalList.Count < 0)
            {
                var message = "user has already items in his previous logged in cart and those are being merged";
            }
            else
            {
                var message = "new car in insert";
            }
