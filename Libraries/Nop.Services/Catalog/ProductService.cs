...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        int GetPreferredTierPriceFlag = 0;
        ...
        public virtual TierPrice GetPreferredTierPrice(Product product, Customer customer, int storeId, int quantity)
        {
            ...
            // Tier pricing upd, Aug 19, 22 Start (Aug 27, 22)

            var prices = actualTierPrices.ToList();

            decimal _price = 0.0m;

            // Package quantities in cart
            string qty1 = "", qty2 = "", qty3 = "", qty4 = "";
            decimal pri = 0.0m;

            // 1 - 14
            if (quantity < prices[1].Quantity && quantity != 0 && GetPreferredTierPriceFlag < 1)
            {
                _price = prices[0].Price;

                qty1 = "1"; //quantity.ToString();
            }

            // 15 - 59
            else if (quantity < prices[2].Quantity && GetPreferredTierPriceFlag < 1)
            {
                _price = prices[1].Price;
            ...
            // 60 - 119
            else if (quantity < prices[3].Quantity && GetPreferredTierPriceFlag < 1)
            {
                _price = prices[2].Price;
            ...
            if (_price > 0)
            {
                //tierPrice.Price = _price;

                // Package quantities in cart - session
                // Popup pricing
                _httpContextAccessor.HttpContext.Session.SetString("package_quantities",
                    qty1.ToString() + "," +
                    qty2.ToString() + "," +
                    qty3.ToString() + "," +
                    qty4.ToString() + "," +
                    ...
                    prices[1].Price.ToString() + "," +
                    prices[2].Price.ToString() + "," +
                    prices[3].Price.ToString() + "," + 
                    prices[0].Quantity.ToString() + "," +
                    prices[1].Quantity.ToString() + "," +
                    prices[2].Quantity.ToString() + "," +
                    prices[3].Quantity.ToString() + ","
                );
            }

                  GetPreferredTierPriceFlag++;
