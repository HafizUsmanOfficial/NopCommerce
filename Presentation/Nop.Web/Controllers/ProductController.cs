...
namespace Nop.Web.Controllers
{
    public partial class ProductController : BasePublicController
    {
        ...
        public virtual IActionResult ProductDetails(int productId, int updatecartitemid = 0)
        {
            ...
            //Opening logged in user’s previous cart item in case new not selected
            ShoppingCartItem updatecartitem = null;

            var cart = _shoppingCartService.GetShoppingCart(_workContext.CurrentCustomer, storeId: _storeContext.CurrentStore.Id);

            // If user logged in
            if (_shoppingCartSettings.AllowCartItemEditing && _workContext.CurrentCustomer.IsRegistered())
            {
                if (cart != null && cart.Count > 0)
                {
                    updatecartitemid = cart[0].Id;
                }
            }

            else if (_shoppingCartSettings.AllowCartItemEditing && !_workContext.CurrentCustomer.IsRegistered())
            {
                if (cart != null && cart.Count > 0)
                {
                    updatecartitemid = cart[0].Id;
